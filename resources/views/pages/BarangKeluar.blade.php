    ...
    <div class="container">
        <div class="card shadow mb-4">
            <div class="card-header bg-white position-relative py-3">
                ...
                <h3 class="m-0 text-center header-center">Transaksi Barang Keluar</h3>
            </div>

            <div class="card-body p-4">
                <div class="table-responsive">
                    @include('partial.NotifMessages')
                    <table id="tblBarangKeluar" class="table table-striped table-bordered w-100">
                        <thead>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="#" class="btn btn-sm btn-outline-primary me-1" title="Edit"
                                            data-bs-toggle="modal" data-bs-target="#modalEdit-{{ $rows->id_brg_keluar }}">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        @include('partial.ModalEditBarangKeluar', ['rows' => $rows])

                                        @if (Auth::user()->role === 'admin' && (int) ($rows->is_approved ?? 0) === 0)
                                            <form action="{{ route('barang-keluar.approve', $rows->id_brg_keluar) }}"
                                            ...
                                            <form action="{{ route('barang-keluar.notApprove', $rows->id_brg_keluar) }}"
                                        ...
                                        {{-- Tombol Hapus --}}
                                        <form action="{{ route('barang-keluar.destroy', $rows->id_brg_keluar) }}"
                                            method="post" class="d-inline">
                                            @csrf
                                            @method('DELETE')
...
@push('modals')
    @include('partial.ModalBarangKeluar')
@endpush
...
@section('scripts')
    <script>
        // ATUR DATATABLES
        $(function() {
            const dt = $('#tblBarangKeluar').DataTable({
                scrollX: false,
                pageLength: 10,
                paging: true,
                searching: true,
            ...
            // Hover effect rows
            $('#tblBarangKeluar tbody tr').hover(
                function() {
        ...
        $('#tblBarangKeluar').on('draw.dt', function() {
        ...
        document.addEventListener('change', function(e) {
            ...
            const val = opt ? (opt.getAttribute(cfg.attr) || '') : '';
            const tgt = document.getElementById(cfg.input);
