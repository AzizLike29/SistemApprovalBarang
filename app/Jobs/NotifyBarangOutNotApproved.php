...
class NotifyBarangOutNotApproved implements ShouldQueue
{
    ...
    public function __construct(int $id_brg_keluar, int $approver_id)
    {
    ...
    public function handle()
    {
        ...
        $textPesan = "Barang Keluar Tidak Disetujui\n"
            . "ID : {$barang->id_brg_keluar}\n"
            ...
            if ($email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL)) {
                Mail::raw($textPesan, function ($m) use ($admin, $email, $fromAddress, $fromName) {
                    $m->from($fromAddress, $fromName)
                        ...
                        ->subject('Barang Keluar Tidak Approved');
                });
            }
        }
